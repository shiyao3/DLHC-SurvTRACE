...
from pycox.preprocessing.discretization import (make_cuts, IdxDiscUnknownC, _values_if_series, DiscretizeUnknownC, Duration2Idx)
...
def hdf_to_csv(path, key):
    """Convert hdf format to csv."""
    if key is None:
        df = pd.read_hdf(path)
    else:
        key = sys.argv[2]
